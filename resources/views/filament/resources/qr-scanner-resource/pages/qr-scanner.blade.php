...
this.showError('No se pudo iniciar la cámara. Verifica los permisos y que tu dispositivo tenga cámara.');
}
}
async stopCamera() {
...
this.showError('No se pudo leer el archivo QR.');
} finally {
...
try {
const response = await fetch('/api/recent-visitor-scans');
...
// Usar notificación toast de Filament
...
// Para loading, usar toast más discreto
this.showFilamentToast('info', 'Procesando', 'Procesando código QR...');
}

showFilamentToast(type, title, message) {
// Crear toast personalizado con estilos inline para asegurar que funcionen
const toast = document.createElement('div');
const toastId = 'toast-' + Date.now();
toast.id = toastId;

// Estilos base inline para asegurar que funcionen
const baseStyles = {
position: 'fixed',
top: '24px',
right: '24px',
...
// Hacer el toast específico clicable
